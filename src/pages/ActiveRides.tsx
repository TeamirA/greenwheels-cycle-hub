...
import { Bike as BikeType, Reservation as ReservationType } from '@/types';
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-2xl font-bold text-graydark">Active Rides</h1>
            <p className="text-muted-foreground">Currently ongoing bike rides in the system</p>
          </div>
        </div>
        <Button 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-gray-500">Short Rides (&lt;15m)</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-greenprimary">{shortRides}</div>
            <div className="w-full h-2 bg-gray-100 rounded-full mt-2">
              <div 
                className="h-full bg-greenprimary rounded-full" 
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-gray-500">Medium Rides (15-30m)</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-greenaccent">{mediumRides}</div>
            <div className="w-full h-2 bg-gray-100 rounded-full mt-2">
              <div 
                className="h-full bg-greenaccent rounded-full" 
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-gray-500">Long Rides (&gt;30m)</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <div className="bg-white rounded-lg shadow">
        <div className="p-4 border-b border-gray-100">
          <h2 className="text-lg font-semibold text-graydark">Current Active Rides</h2>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {activeReservations.map((reservation) => {
                ...
                return (
                  <tr key={reservation.id} className={`hover:bg-gray-50 ${getRowClass(reservation.startTime)}`}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-8 w-8 rounded-full bg-gray-200 flex items-center justify-center">
                          <User size={16} className="text-graydark" />
                        </div>
                        <div className="ml-3">
                          <div className="font-medium text-graydark">{user?.name || 'Unknown'}</div>
                          <div className="text-xs text-gray-500">ID: {reservation.userId.slice(0, 8)}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-graydark">
                      <div className="flex items-center">
                        <Bike size={16} className="mr-1 text-graydark" />
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-graydark">
                      <div className="flex items-center">
                        <Clock size={16} className="mr-1 text-graydark" />
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-lg font-semibold text-graydark mb-4">Ride Locations</h2>
        <div className="h-[400px] bg-gray-100 rounded-lg flex items-center justify-center">
          <div className="text-center">
            <MapPin size={32} className="mx-auto text-greenprimary mb-2" />
            <p className="text-graydark">Interactive ride map would be displayed here</p>
          </div>
        </div>
      </div>
    </div>
